...
class Exporter {
  ...
  addCard(fieldsObj, { tags } = {}) {
    ...
    const fieldValues = [
      ...
      fieldsObj.Meaning || '',
      fieldsObj.Audio || '',
      fieldsObj.AudioButtons || '',
      fieldsObj.Meta || ''
    ];
    const joinedFields = fieldValues.join(separator);
    const sfld = fieldValues[0];
    
    const note_guid = this._getNoteGuid(topDeckId, fieldValues[0], fieldValues[1]);
